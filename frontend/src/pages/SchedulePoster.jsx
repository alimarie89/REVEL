...
function DayScheduleGrid({ day }) {
  ...
  // Get events for time bucket and space, preferring narrower buckets
  const getEventsForTimeAndSpace = (timeBucket, space) => {
    const allMatches = dayEvents.filter(event => 
      eventFallsInTimeBucket(event.time, timeBucket) && normalizeVenue(event.space) === space
    );
    
    // Filter out events that would fit better in a narrower bucket
    const curated = getCuratedTimes();
    const timeBucketIndex = curated.indexOf(timeBucket);
    
    return allMatches.filter(event => {
      // Check if this event would fit better in a narrower bucket listed after this one
      for (let i = timeBucketIndex + 1; i < curated.length; i++) {
        const laterBucket = curated[i];
        if (eventFallsInTimeBucket(event.time, laterBucket)) {
          // This event fits in a later (narrower) bucket, skip it here
          return false;
        }
      }
      return true;
...
function DayGrid({ day }) {
  ...
  const getCuratedTimes = () => {
    if (day === 'Thursday 7/2') {
      ...
      return ['8:15–9:30 AM', '10:00 AM–12:00 PM', '1:30–3:30 PM', '2:30–3:30 PM', '4:00–6:00 PM', '7:15–9:15 PM', '9:45 PM–1:00 AM'];
